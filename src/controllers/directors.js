import { Directors, Contents } from "../models/directors";

//{ Crea un director
export const createDirector = async (req, res) => {
   const { name } = req.body;
   try{
      let newDirector = await Actors.create({ name });
...
export const editDirectorById = async (req,res) => {
    try{  
          ...
          const results = await Directors.update({ where: { id }, attributes: ["id", "name"] });
          return res.status(201).json({
            results,
